<template>
  <div class="min-h-screen bg-white p-6">
    ...
    <p class="text-gray-700 mb-2">
      Nama: <span class="font-medium">{{ mahasiswa?.nama }}</span><br>
      NIM: <span class="font-medium">{{ mahasiswa?.nim }}</span>
    </p>
...
const getMahasiswa = async () => {
  try {
    const res = await axios.get(`http://localhost:8000/api/mahasiswa/${mahasiswaId}`, {
...
const loadData = async () => {
  const mahasiswaId = mahasiswa.value.id
  ...
  try {
    ...
    const response = await axios.get(`http://localhost:8000/api/admin/mahasiswa-mata-kuliah/${mahasiswaId}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const submitMataKuliah = async () => {
  ...
  try {
    const payload = {
      mahasiswa_id: mahasiswa.value.id,  // Menggunakan mahasiswa.id dari state
      mata_kuliah_ids: selectedMataKuliah.value.map(item => item.id)
    }

    await axios.post('http://localhost:8000/api/admin/mahasiswa-mata-kuliah', payload, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const removeMataKuliah = async (mataKuliahId) => {
  const mahasiswaId = mahasiswa.value.id  // Menggunakan mahasiswa.id dari state
  if (!mahasiswaId) return
  ...
  try {
    await axios.delete(`http://localhost:8000/api/admin/mahasiswa-mata-kuliah/${mahasiswaId}`, {
      headers: { Authorization: `Bearer ${token}` },
